...
export default function IndustryList() {
  ...
  const {
    ...
    itemsLimit,
    refetch,
  } = usePaginatedList<IndustryRecord>(fetchIndustryList);

  const industriesRef = useRef<IndustryRecord[]>(industries);
  useEffect(() => {
  ...
  const handleSortOrderChange = (item: IndustryRecord, delta: number) => {
  ...
  const handleSortOrder = async (itemId: number) => {
    const latestItem = industriesRef.current.find((i) => i.id === itemId);
    ...
    try {
      await updateIndustrySortOrder(latestItem, finalSortOrder);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteIndustry(deleteItemId);
  ...
  const confirmStatusToggle = async () => {
    ...
    try {
      ...
      await toggleIndustryStatus(item, newStatus);
      ...
      toast({ title: "Success", description: "Industry status updated successfully" });
    } catch (error) {
      toast({
        title: "Error",
        description:
  ...
  const columns: ColumnDef<IndustryRecord>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <RowActionsMenu
            onEdit={() => navigate(`/industry/${item.id}/edit`)}
