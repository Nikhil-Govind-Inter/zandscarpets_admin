...
export default function FloatingIconList() {
  ...
  const floatingIconItemsRef = useRef<FloatingIcon[]>(floatingIconItems);
  useEffect(() => {
    floatingIconItemsRef.current = floatingIconItems;
  }, [floatingIconItems]);
  ...
  const confirmStatusToggle = async () => {
    ...
    try {
      ...
      await toggleFloatingIconStatus(item, newStatus);

      // Optimistic update for instant feedback...
      setFloatingIconItems((prev) =>
        prev.map((i) =>
      ...
      toast({
        title: "Success",
        description: "Floating icon status updated successfully",
      });
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to update floating icon status",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSortOrderChange = (item: FloatingIcon, delta: number) => {
  ...
  const handleSortOrder = async (itemId: number) => {
    ...
    const latestItem = floatingIconItemsRef.current.find((i) => i.id === itemId);
    ...
    try {
      await updateFloatingIconSortOrder(latestItem, finalSortOrder);
      toast({
        title: "Success",
