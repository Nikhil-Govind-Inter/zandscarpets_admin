...
export default function ServiceForm() {
  ...
  const loadServiceData = async (itemId: number) => {
    try {
      setInitialLoading(true);
      const response = await fetchServiceById(itemId);
      ...
      toast({
        title: "Error",
        description: error instanceof ApiError ? error.message : "Failed to load service data",
        variant: "destructive",
      });
    } finally {
  ...
  const onSubmit = async (data: ServiceFormData) => {
    try {
      ...
      if (isEditing && id) {
        await updateService(parseInt(id), payload);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">{isEditing ? "Edit" : "Add"} Service</h1>
          <p className="text-muted-foreground">
            {isEditing ? "Update" : "Create a new"} service
          </p>
        </div>
      </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., Interior Design, Renovation" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
