...
export default function MessagesList() {
  ...
  const {
    ...
    searching,
    refetch,
  } = usePaginatedList<MessagesRecord>(fetchMessagesList);
  ...
  const itemsRef = useRef<MessagesRecord[]>(items);
  useEffect(() => {
  ...
  const confirmStatusToggle = async () => {
    ...
    try {
      ...
      await toggleMessagesStatus(item, newStatus);
      ...
      toast({
        title: "Error",
        description: error instanceof ApiError ? error.message : "Failed to update status",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSortOrderChange = (item: MessagesRecord, delta: number) => {
    ...
    setItems((prev) => prev.map((i) => (i.id === item.id ? { ...i, sort_order: newSortOrder } : i)));
  ...
  const handleSortOrder = async (itemId: number) => {
    const latestItem = itemsRef.current.find((i) => i.id === itemId);
    ...
    try {
      await updateMessagesSortOrder(latestItem, finalSortOrder);
      toast({ title: "Success", description: "Sort order updated successfully" });
    } catch (error) {
      setItems((prev) => prev.map((i) => (i.id === itemId ? { ...i, sort_order: originalSortOrder } : i)));
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteMessages(deleteItemId);
      ...
      toast({
        title: "Error",
        description: error instanceof ApiError ? error.message : "Failed to delete message",
        variant: "destructive",
      });
    } finally {
  ...
  const columns: ColumnDef<MessagesRecord>[] = [
    {
      id: "id",
      header: "ID",
      cell: ({ row }) => (
        <div className="font-mono text-sm">{(itemsPage - 1) * itemsLimit + row.index + 1}</div>
      ),
    },
    {
      ...
      header: "Photo",
      cell: ({ row }) => (
        <MediaThumbnail path={row.getValue("media_path")} alt={row.original.name} />
      ),
    },
    {
      accessorKey: "name",
    ...
    },
    {
      ...
      header: "Organization",
      cell: ({ row }) => <div className="text-sm">{row.getValue("Organization") || "-"}</div>,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <RowActionsMenu
            onEdit={() => navigate(`/about-messages/${item.id}/edit`)}
  ...
  return (
    <>
      <DataTable
        ...
        onAdd={() => navigate("/about-messages/new")}
