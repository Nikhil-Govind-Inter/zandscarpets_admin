...
import {
  fetchHomeBannerSliders,
  deleteHomeBannerSlider,
  HomeBannerSlider,
...
export default function HomeBannerSliderList() {
  ...
  const [sliders, setSliders] = useState<HomeBannerSlider[]>([]);
  ...
  useEffect(() => {
    if (debouncedSearchQuery !== searchQuery) return; // Only reset when debounce is complete
    setCurrentPage(1);
  }, [debouncedSearchQuery]);

  // Load sliders when page or debounced search changes
  useEffect(() => {
    loadSliders();
  ...
  const loadSliders = async () => {
    try {
      ...
      const response = await fetchHomeBannerSliders(
        currentPage,
        pageSize,
      ...
      toast({
        title: "Error",
        description: "Failed to load home banner sliders",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteHomeBannerSlider(deleteItemId);
      setSliders((prev) => prev.filter((item) => item.id !== deleteItemId));
      ...
      toast({
        title: "Error",
        description: "Failed to delete banner slider",
        variant: "destructive",
      });
    } finally {
  ...
  const columns: ColumnDef<HomeBannerSlider>[] = [
    {
      accessorKey: "id",
      header: "ID",
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          {row.original.media_desktop_path ? (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage banner slider content for the homepage
          </p>
        </div>
      </div>
      ...
      <DataTable
        ...
        data={sliders}
        title=""
        searchPlaceholder="Search banner sliders..."
        onAdd={() => navigate("/home-banner-slider/create")}
        addButtonText="Add Banner Slider"
        loading={loading}
        searching={searching}
        pagination={{
          pageSize,
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              banner slider and remove its data from the servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
